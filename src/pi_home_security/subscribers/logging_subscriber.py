...
def logging_event_listener(device: Dict[str, Any]):
    if not device:
        return

    if device.get("state") == "open":
        print(f"📄 log state triggered: {device.get('name')} is OPEN 👋")
        # Add actual alarm trigger logic here
    elif device.get("state")== "closed":
        print(f"📄 log state triggered: {device.get('name')} is CLOSED 🚪")